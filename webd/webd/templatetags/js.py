...
from aaew_linggloss import computeLingGlossing
...
@register.filter(is_safe=True)
def MdCtoRES(codeStr):
    if codeStr:
        codeStr = codeStr.strip()
        codeStr = codeStr.replace('_', '') 
        
        codeStr = codeStr.replace('[(', '%%%(%%%')   # nicht in BTS
        codeStr = codeStr.replace(')]', '%%%)%%%')  
        codeStr = codeStr.replace('[&', '%%%〈%%%')  # nicht in BTS
        codeStr = codeStr.replace('&]', '%%%〉%%%') 
        codeStr = codeStr.replace('[{', '%%%{%%%')  # nicht in BTS
        codeStr = codeStr.replace('}]', '%%%}%%%') 
        codeStr = codeStr.replace("['", '%%%|%%%:.') # Wortaraund 
        codeStr = codeStr.replace("']", '%%%|%%%:.') 
        codeStr = codeStr.replace('[?', '⸢') 
        codeStr = codeStr.replace('?]', '⸣') 
        codeStr = codeStr.replace('[[', '%%%[%%%') # nicht in BTS
        ...
        codeStr = codeStr.replace('**', '-')   # Leipzig, JSesh placement information
        codeStr = re.sub(r'{{[0-9,]+}}', '', codeStr)  # Leipzig, JSesh placement information
        codeStr = codeStr.replace('&&', '-')  # ?
        codeStr = codeStr.replace('&', '-')   # nicht im BTS
        #codeStr = codeStr.replace('(', '')  # Leipzig
        #codeStr = codeStr.replace(')', '')  
        #codeStr = codeStr.replace('*', '-')  # Leipzig
        #codeStr = codeStr.replace(':', '-')  # Leipzig
        
        codeStr = codeStr.replace('..', 'empty')
        codeStr = codeStr.replace('.', 'empty[width=0.5,height=0.5]') # nicht in BTS
        
        codeStr = codeStr.replace('//', 'empty[shade]') # 'empty[width=0.5,shade]-"."-"."-empty[width=0.5,shade]') 
        codeStr = codeStr.replace('h/', 'empty[width=0.5,shade]')  # nicht in BTS
        codeStr = codeStr.replace('v/', 'empty[height=0.5,shade]')  # nicht in BTS
        codeStr = codeStr.replace('/', 'empty[width=0.5,height=0.5,shade]')  # nicht in BTS

        codeStr = codeStr.replace('"?"', '%%%?%%%') 
        codeStr = codeStr.replace('"¿"', '%%%¿%%%') 
        codeStr = codeStr.replace('"lb"', '%%%|%%%[gray]^%%%lb%%%[gray]')   # nicht in Grundregelwerk
        codeStr = re.sub('[\-\*\:]"(var|mutil|monogr|composite|large|hierat|hiero|demot|elab|abbr|ligstart|ligend)"', r'\^%%%\1%%%[gray]', codeStr) 
        def stringHelper(latinWord):
            ...
            if latinWord:
                ...
                charList = '"'+'"[scale=0.4,gray]-"'.join(latinChars)+'"[scale=0.4,gray]'
            return charList
        codeStr = re.sub(r'"[^"]+?"', lambda m: stringHelper(m), codeStr)   # nicht in Grundregelwerk
        codeStr = codeStr.replace('%%%', '"')
        
        codeStr = codeStr.replace('\\r1', '[rotate=270]')   # nicht in Grundregelwerk
        codeStr = codeStr.replace('\\r2', '[rotate=180]')
        codeStr = codeStr.replace('\\r3', '[rotate=90]')
        codeStr = codeStr.replace('\\t1', '[mirror,rotate=90]')  
        codeStr = codeStr.replace('\\t2', '[mirror,rotate=180]')
        codeStr = codeStr.replace('\\t3', '[mirror,rotate=270]')
        codeStr = codeStr.replace('\\red', '[red]')
        codeStr = codeStr.replace('\\i', '[gray]')
        codeStr = codeStr.replace('\\l', '')
        codeStr = re.sub(r'\\R(\-?[0-9]+)', r'[rotate=\1]', codeStr)   # nicht in Grundregelwerk
        codeStr = re.sub(r'\\([0-9]+)', r'[scale=\1]', codeStr)   # nicht im BTS
        codeStr = codeStr.replace('\\', '[mirror]')   # nicht in Grundregelwerk
        
        codeStr = re.sub(r'\<[Ff]\-([^\>]+)\-[Ff]?\>', r'inb(\1)', codeStr) # nicht in BTS
        
        codeStr = re.sub(r'\<[Ss]0\-([^\>]+)\-[Ss]?0\>', r'modify[before=0.2,after=0.6,omit](serekh(empty[width=0.5,height=0.5]-\1-empty[width=0.5,height=0.5]))', codeStr) 
        codeStr = re.sub(r'\<[Ss]1\-([^\>]+)\-[Ss]?0\>', r'modify[after=0.6,omit](serekh(\1-empty[width=0.5,height=0.5]))', codeStr) 
        codeStr = re.sub(r'\<[Ss]0\-([^\>]+)\-[Ss]?2\>', r'modify[before=0.2,omit](serekh(empty[width=0.5,height=0.5]-\1))', codeStr) 
        codeStr = re.sub(r'\<[Ss]\-([^\>]+)\-[Ss]?\>', r'serekh(\1)', codeStr) 
        codeStr = re.sub(r'\<[Ss]2\-([^\>]+)\-[Ss]?1\>', r'serekh[mirror](\1)', codeStr) 
        #codeStr = re.sub(r'\<[Ss]1?\-', 'modify[after=1,omit](serekh(empty[width=0.5,height=0.5]))-', codeStr) # fallback
        #codeStr = re.sub(r'\-[Ss]2\>', '-modify[before=1,omit](serekh(empty[width=0.5,height=0.5]))', codeStr)
        ...
        codeStr = re.sub(r'\<[Hh]2\-([^\>]+)\-[Hh]?0\>', r'modify[after=0.2,omit](Hwtopenunder(\1-empty[width=0.5,height=0.5]))', codeStr) 
        ...
        codeStr = re.sub(r'\<0\-([^\>]+)\-2\>', r'modify[before=0.2,omit](cartouche(empty[width=0.5,height=0.5]-\1))', codeStr) 
        codeStr = re.sub(r'\<2\-([^\>]+)\-0\>', r'modify[after=0.2,omit](cartouche[mirror](\1-empty[width=0.5,height=0.5]))', codeStr) 
        codeStr = re.sub(r'\<0\-([^\>]+)\-1\>', r'modify[before=0.2,omit](cartouche[mirror](empty[width=0.5,height=0.5]-\1))', codeStr) 
        codeStr = re.sub(r'\<1\-([^\>]+)\-1\>', r'oval(\1)', codeStr) 
        codeStr = re.sub(r'\<2\-([^\>]+)\-1\>', r'cartouche[mirror](\1)', codeStr) 
        codeStr = re.sub(r'\<\-([^\>]+)\-\>', r'cartouche(\1)', codeStr) 
        codeStr = re.sub(r'\<1?\-0\>', 'modify[after=1,omit](cartouche(empty[width=0.5,height=0.5]))', codeStr) 
        codeStr = re.sub(r'\<0\-2?\>', 'modify[before=1,omit](cartouche(empty[width=0.5,height=0.5]))', codeStr)

        codeStr = codeStr.replace('S56', 'S7')
        codeStr = codeStr.replace('D153', 'D26')
        codeStr = codeStr.replace('M22A', 'M22*M22')
        codeStr = codeStr.replace('O38A', 'O38[mirror]')
        codeStr = codeStr.replace('R8A', 'R8*[sep=0,fix]R8*[sep=0,fix]R8')
        
        codeStr = re.sub(r'([0-9])([A-Z])', lambda m: m.group(0).lower(), codeStr) #replace alphanumerical glyph index by lowercase

        codeStr = re.sub(r'\bo\b', '"°"[red]', codeStr)
        codeStr = re.sub(r'\bO\b', '"°"', codeStr) 
    return codeStr 
...
niceRelationNameDict = {
        'root': 'Root',
        'rootOf': 'Root of',
        'predecessor': 'Historical predecessors',
        'successor': 'Historical successors',
        'referencing': 'Substituted by',        
        'referencedBy': 'Substitutes for following obsolete lemmata',       
        'composedOf': 'Parts',        
        'composes': 'Part of',        
        'contains': 'Subordinates',        
        'partOf': 'Superordinates',      
        }      
@register.filter(is_safe=True)
def niceRelationName(name):
    newName = niceRelationNameDict.get(name)
    if newName:
        return newName
    else:
        return name
...
def niceQueryData(name):
    newName = niceQueryDataDict.get(name)
    if newName:
        return newName
    else:
        return name
...
def niceReviewState(name):
    newName = niceReviewStateDict.get(name)
    if newName:
        return newName
    else:
        return name
...
@register.filter(is_safe=True)
def niceYear(yearStr):
    try:
        year = int(yearStr)
        if year <= 0:
            yearStr= str(-year) + " BCE"
        else:
            yearStr= str(year) + " CE"
    except:
        yearStr ='(none)'
...
def verbalFlexcode(flexcode):
    verbal = verbalFlexcodeDict.get(flexcode)
    if verbal:
        return verbal
    else:
...
register.simple_tag(computeLingGlossing)
...
def replaceGardiner (signs):     
    for i in range (0, len(signs)):
        unicodeSign = dictUnicodeHieroglyphs.get(signs[i])
        if unicodeSign:
            signs[i] = unicodeSign
        else:
